	...
	public function save_settings()
	{
		if (isset($_POST['save_api_settings'])) {
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'You do not have sufficient permissions to access this page.', Contact_FormSI_TXT ) );
			}
			
			// Verify nonce
			if ( ! isset( $_POST['cf7isi_settings_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['cf7isi_settings_nonce'] ) ), 'cf7isi_save_settings' ) ) {
				wp_die( __( 'Security check failed.', Contact_FormSI_TXT ) );
			}
			
			$api_token = isset( $_POST['api_token'] ) ? trim( sanitize_text_field( wp_unslash( $_POST['api_token'] ) ) ) : '';
			$sender_id = isset( $_POST['sender_id'] ) ? trim( sanitize_text_field( wp_unslash( $_POST['sender_id'] ) ) ) : '';
			$country = isset( $_POST['country'] ) ? trim( sanitize_text_field( wp_unslash( $_POST['country'] ) ) ) : '';
			$country_code = isset( $_POST['country_code'] ) ? trim( sanitize_text_field( wp_unslash( $_POST['country_code'] ) ) ) : '';
			$reg_phone = isset( $_POST['reg_phone'] ) ? trim( sanitize_text_field( wp_unslash( $_POST['reg_phone'] ) ) ) : '';

            if (empty($api_token) && empty($sender_id)) {
                wp_die( __( 'API Token and Sender ID are required!', Contact_FormSI_TXT ) );
            }
