...
class USMSGH_Contact_Form_Sms_Notification_abn_Functions
{
    ...
    public function send_sms($phone, $message)
    {
        ...
        if (!empty($api_token) && !empty($sender_id)) {
            ...
            // Normalize phone numbers to ensure they have the 233 country code
            $normalized_phones = array();
            foreach ($phones as $p) {
                $p = trim($p);
                ...
                if (strpos($p, '0') === 0) {
                    $p = $country_code . substr($p, 1);
                }
                // If doesn't start with country code, prepend it
                elseif (strpos($p, $country_code) !== 0) {
                    $p = $country_code . $p;
                }
            ...
            if (!empty($normalized_phones)) {
                $sms->send_sms($endpoint, $api_token, $sender_id, $normalized_phones, $message);
            }
        }
        return false;
    }
